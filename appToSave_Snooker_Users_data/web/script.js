...
document.getElementById('submit').onclick = function () {
    ...
    if (validatePhoneNumber(mobile)){
        ...
        // storing data in python eel
        eel.send_data(name_=name_, mobile=mobile, board_no=board_no, board_type=type_of_board[board_no], price=prices_[type_of_board[board_no]])(function(output){
            console.log(output)
        })
    }
...
window.addEventListener("keydown", function(e) {
    if ((e.key === "r" || e.key === "F5") && (e.ctrlKey || e.metaKey)) {
        e.preventDefault();
    }
});
